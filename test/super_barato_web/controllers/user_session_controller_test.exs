defmodule SuperBaratoWeb.UserSessionControllerTest do
  use SuperBaratoWeb.ConnCase, async: true

  import SuperBarato.AccountsFixtures
  ...
  describe "POST /users/log-in - email and password" do
    test "logs the user in", %{conn: conn, user: user} do
      ...
      # Confirm the session is actually established by hitting a page
      # that requires authentication.
      assert html_response(get(conn, ~p"/users/settings"), 200) =~ user.email
    end
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_super_barato_web_user_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
  ...
  describe "POST /users/log-in - magic link" do
    test "sends magic link email when user exists", %{conn: conn, user: user} do
      ...
      assert SuperBarato.Repo.get_by!(Accounts.UserToken, user_id: user.id).context == "login"
    end
