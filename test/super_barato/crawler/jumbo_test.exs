defmodule SuperBarato.Crawler.JumboTest do
  ...
  describe "Cencosud.extract_render_data/1" do
    test "pulls window.__renderData out of the home page" do
      ...
      assert {:ok, %{"menu" => _}} = Cencosud.extract_render_data(html)
    end

    test "errors when __renderData is missing" do
      assert {:error, :no_render_data} =
               Cencosud.extract_render_data("<html><body>no render data here</body></html>")
    end
  end

  describe "Cencosud.categories_from_render_data/2 (Jumbo home fixture)" do
    setup do
      html = Fixtures.read!(:jumbo, "home.html")
      {:ok, data} = Cencosud.extract_render_data(html)
      {:ok, cats} = Cencosud.categories_from_render_data(:jumbo, data)
      {:ok, cats: cats}
    end
